...
const languagePatterns_1 = require("./languagePatterns");
/**
...
class CodeParser {
     ...
     */
    static extractFunctions(sourceCode, fileName = 'unknown') {
        const functions = [];
        const lines = sourceCode.split('\n');
        // Check if we have language-specific patterns
        const languagePattern = (0, languagePatterns_1.getLanguagePattern)(fileName);
     ...
     */
    static extractJavaScriptFunctions(sourceCode, fileName) {
        const functions = [];
        const lines = sourceCode.split('\n');
        try {
     ...
     */
    static extractFunctionsWithLanguagePatterns(sourceCode, languagePattern) {
        const functions = [];
        const lines = sourceCode.split('\n');
            ...
            for (const pattern of languagePattern.functionPatterns) {
                ...
                if (match) {
                    ...
                    if (languagePattern.blockDelimiters.end === 'indent') {
                        ...
                        endLine = (0, languagePatterns_1.findPythonFunctionEnd)(lines, i);
                    }
                    else {
     ...
     */
    static extractFunctionsWithRegex(sourceCode) {
        const functions = [];
        const lines = sourceCode.split('\n');
     ...
     */
    static findFunctionAtLines(functions, startLine, endLine) {
     ...
     */
    static findFunctionByName(functions, functionName) {
     ...
     */
    static findHistoricalMatch(historicalContent, fileName, targetStartLine, targetEndLine, currentSelection, currentFunction) {
     ...
     */
    static compareWithHistorical(currentContent, currentFileName, historicalContent, selectedStartLine, selectedEndLine) {
